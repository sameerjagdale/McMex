...
public:
	...
	virtual const long* getDataSize() const {
		return NULL;
	}
...
// Static version of the conversion method
	static DataObject* convertType(const DataObject* pObject,
			DataObject::Type outType) {
		return pObject->convert(outType);
	}
...
// Method to test whether or not this object is a matrix object
	bool isMatrixObj() const {
		return m_type >= MATRIX_I32 && m_type <= CELLARRAY;
	}
